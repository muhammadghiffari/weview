/**
 * WeView Observatory — Main Scene Orchestrator
 *
...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('weview-settings-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('weview-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('weview-observatory-settings');
        localStorage.setItem('weview-settings-version', SETTINGS_VERSION);
      }
    } catch {}
  ...
  _autoDetectLive() {
    // Skip health-check probing — connect directly to the sensing server WebSocket.
    // The server runs on port 8765 by default.
    const host = window.location.hostname || 'localhost';
    const wsUrl = `ws://${host}:8765/ws/sensing`;
    console.log('[Observatory] Auto-connecting to', wsUrl);
  ...
  _connectWS(url) {
    ...
    // Accept both ws:// direct URL and http:// base URL
    const wsUrl = url.startsWith('ws') ? url : url.replace(/^http/, 'ws') + '/ws/sensing';
    console.log('[Observatory] Connecting WebSocket to', wsUrl);
    try {
      this._ws = new WebSocket(wsUrl);
      this._ws.onopen = () => {
        console.log('[Observatory] WebSocket connected ✓ — switching to LIVE mode');
        // CRITICAL: switch dataSource so animation loop uses live data
        this.settings.dataSource = 'ws';
        this._hud.updateSourceBadge('ws', this._ws);
        // Sync the settings UI dropdown
      ...
      this._ws.onmessage = (evt) => {
        try {
          ...
          // Server sends "msg_type" field, not "type" — check both
          const msgType = msg.msg_type || msg.type || '';
          if (msgType !== 'sensing_update') return;
          // Transform sensing_update → observatory data format
      ...
      this._ws.onerror = (e) => {
        console.warn('[Observatory] WebSocket error', e);
      };
    } catch(e) {
      console.warn('[Observatory] Failed to open WebSocket', e);
    }
  }

  /**
   * Transform a backend sensing_update into the observatory data format.
   * Backend format: { estimated_persons, classification, features, vital_signs, persons, signal_field }
   * Observatory format: same but persons need position/pose/motion_score arrays.
   */
  _transformSensingUpdate(msg) {
    const estimated = msg.estimated_persons || 0;
    const cls = msg.classification || {};
    const feat = msg.features || {};

    // Build persons array with 3D positions spread across the room
    let persons = [];
    if (Array.isArray(msg.persons) && msg.persons.length > 0) {
      // Server sent actual person detections with keypoints
      persons = msg.persons.map((p, i) => {
        const kps = p.keypoints || [];
        // Extract approximate 3D position from keypoints (hip midpoint)
        const lhip = kps.find(k => k.name === 'left_hip');
        const rhip = kps.find(k => k.name === 'right_hip');
        let px = 0, pz = 0;
        if (lhip && rhip) {
          // Convert from 2D canvas coords (0-640 x 0-480) to 3D room coords (-5 to 5)
          px = ((lhip.x + rhip.x) / 2 / 640 - 0.5) * 8;
          pz = ((lhip.y + rhip.y) / 2 / 480 - 0.5) * 6;
        } else {
          // Spread persons evenly across room when no keypoints
          const spacing = 6 / (estimated + 1);
          px = -3 + spacing * (i + 1);
        ...
        return {
          id: p.id || (i + 1),
          position: [px, 0, pz],
          pose: cls.motion_level === 'present_moving' ? 'walking' :
                cls.motion_level === 'absent' ? 'standing' : 'standing',
          motion_score: feat.motion_band_power ? Math.min(100, feat.motion_band_power * 200) : 20,
          confidence: p.confidence || 0.7,
          keypoints: kps,
        };
      });
    } else if (estimated > 0) {
      // WiFi detected persons but no keypoints — place dots in room
      for (let i = 0; i < Math.min(estimated, 4); i++) {
        const spacing = 6 / (estimated + 1);
        const px = -3 + spacing * (i + 1);
        const pz = (Math.sin(i * 1.3) * 2);
        const isMoving = cls.motion_level === 'present_moving';
        persons.push({
          id: i + 1,
          position: [px, 0, pz],
          pose: isMoving ? 'walking' : 'standing',
          motion_score: isMoving ? 60 : 15,
    ...
    return {
      ...
      source: msg.source || 'esp32',
      tick: msg.tick || 0,
      classification: {
        presence: cls.presence || estimated > 0,
        motion_level: cls.motion_level || (estimated > 0 ? 'present_still' : 'absent'),
        confidence: cls.confidence || 0,
        fall_detected: cls.fall_detected || false,
      },
      features: {
        mean_rssi: feat.mean_rssi || 0,
        variance: feat.variance || 0,
        motion_band_power: feat.motion_band_power || 0,
      },
      vital_signs: msg.vital_signs || {},
