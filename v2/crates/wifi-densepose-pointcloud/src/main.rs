//! weview-pointcloud — real-time dense point cloud from camera + WiFi CSI
//!
//! Pipeline: Camera → Depth → Backproject → Fuse with WiFi occupancy → Stream
//!
//! Usage:
//!   weview-pointcloud serve               # HTTP + Three.js viewer
//!   weview-pointcloud capture --frames 1  # capture to PLY
//!   weview-pointcloud demo                # synthetic demo
//!   weview-pointcloud train               # calibration training
//!   weview-pointcloud csi-test            # send test CSI frames (ADR-018 binary)
...
#[derive(Parser)]
#[command(name = "weview-pointcloud", version = VERSION)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Train {
        #[arg(long, default_value = "~/.local/share/weview/training")]
        data_dir: String,
...
async fn demo() -> Result<()> {
    println!("╔══════════════════════════════════════════════╗");
    println!("║  WeView Dense Point Cloud — Demo             ║");
    println!("╚══════════════════════════════════════════════╝");
    println!();
...
async fn train(data_dir: &str, brain_url: Option<&str>) -> Result<()> {
    println!("╔══════════════════════════════════════════════╗");
    println!("║  WeView Point Cloud — Training               ║");
    println!("╚══════════════════════════════════════════════╝");
    println!();
