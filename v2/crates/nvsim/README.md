...
| **WeView pipeline contributor** | Get a binary `MagFrame` shape (`0xC51A_6E70`) you can plumb into existing observability, with optional ruvector trace compression behind a feature flag |
...
`tracing`, `rand`, and `rand_chacha`. WeView ecosystem integrations
...
- Cryogenic operation — WeView ships room-temperature; `nvsim` follows.
