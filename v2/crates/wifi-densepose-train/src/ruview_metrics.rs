//! WeView three-metric acceptance test (ADR-031).
//!
...
impl std::fmt::Display for WeViewTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            WeViewTier::Fail => write!(f, "FAIL"),
            WeViewTier::Bronze => write!(f, "BRONZE"),
            WeViewTier::Silver => write!(f, "SILVER"),
            WeViewTier::Gold => write!(f, "GOLD"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct WeViewAcceptanceResult {
...
impl WeViewAcceptanceResult {
    /// A human-readable summary of the acceptance test.
    pub fn summary(&self) -> String {
        format!(
            "WeView Tier={} | PCK={:.3} OKS={:.3} | MOTA={:.3} IDsw={} | Breathing={:.1}BPM err",
            self.tier,
...
mod tests {
    ...
    fn tier_determination_gold() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WeViewTier::Gold);
    }
    ...
    fn tier_determination_silver() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WeViewTier::Silver);
    }
    ...
    fn tier_determination_bronze() {
        ...
        let tr = TrackingResult { passes: true, ..Default::default() };
        let vs = VitalSignResult { passes: false, ..Default::default() };
        assert_eq!(determine_tier(&je, &tr, &vs), WeViewTier::Bronze);
    }
    ...
    fn tier_determination_fail() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WeViewTier::Fail);
    }

    #[test]
    fn tier_ordering() {
        assert!(WeViewTier::Gold > WeViewTier::Silver);
        assert!(WeViewTier::Silver > WeViewTier::Bronze);
        assert!(WeViewTier::Bronze > WeViewTier::Fail);
    }
