...
struct AppStateInner {
    ...
    /// Camera hint from MediaPipe (browser-side pose estimation).
    /// `(person_count, confidence)`. Expires after 2 seconds if not refreshed.
    camera_hint: Option<(usize, f64)>,
    camera_hint_expiry: Option<std::time::Instant>,
}
...
// ── Camera hint for hybrid fusion (MediaPipe → WiFi bias) ───────────────────

/// Accepts a camera-derived person count hint from the browser (MediaPipe).
/// The hint is stored with a 2-second TTL and used to bias the WiFi CSI
/// person count estimation in `score_to_person_count_fused()`.
async fn camera_hint_endpoint(
    State(state): State<SharedState>,
    Json(body): Json<serde_json::Value>,
) -> Json<serde_json::Value> {
    let person_count = body.get("person_count")
        .and_then(|v| v.as_u64())
    ...
    let mut s = state.write().await;
    s.camera_hint = Some((person_count.min(3), confidence));
    s.camera_hint_expiry = Some(std::time::Instant::now() + std::time::Duration::from_secs(2));

    Json(serde_json::json!({
        "success": true,
        "accepted": { "person_count": person_count.min(3), "confidence": confidence },
    }))
}
        ...
        match socket.recv_from(&mut buf).await {
            Ok((len, src)) => {
                ...
                if let Some(vitals) = parse_esp32_vitals(&buf[..len]) {
                    ...
                    let now = std::time::Instant::now();
                    let mut total_persons = if vitals.presence {
                        let (fused, fallback_count) = multistatic_bridge::fuse_or_fallback(
                            &s.multistatic_fuser, &s.node_states,
                        );
                        // For ESP32 hardware, the raw uncalibrated amplitudes will break the
                        // simulator-based score logic (mean=20.0). We instead rely entirely
                        // on the hardware's on-device Edge DSP which is already calibrated.
                        let count = fallback_count.unwrap_or(0);
                        s.prev_person_count = count;
                    ...
                    // Hybrid Camera Fusion: Mutual Backup (Max Fusion)
                    // If camera sees fewer people because of occlusion, WiFi count takes over.
                    // If WiFi misses someone who is sitting very still, Camera count takes over.
                    if let Some((cam_count, cam_conf)) = s.camera_hint {
                        if let Some(expiry) = s.camera_hint_expiry {
                            if std::time::Instant::now() < expiry && cam_conf > 0.4 {
                                total_persons = total_persons.max(cam_count);
                            }
                        }
                    }
...
async fn main() {
    ...
    let http_app = Router::new()
        ...
        // Camera hint for hybrid fusion (MediaPipe → WiFi bias)
        .route("/api/v1/camera-hint", post(camera_hint_endpoint))
        // Static UI files
