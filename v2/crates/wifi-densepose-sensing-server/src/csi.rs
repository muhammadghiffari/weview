...
pub fn raw_classify(score: f64) -> String {
    // Thresholds recalibrated for 3-node ESP32-S3N16R8 mesh.
    // Lower presence floor (0.02 vs 0.04) to catch still subjects;
    // 3 overlapping nodes produce more consistent ambient signal,
    // so the false-positive risk at the lower floor is acceptable.
    if score > 0.22 { "active".into() }
    else if score > 0.10 { "present_moving".into() }
    else if score > 0.02 { "present_still".into() }
    else { "absent".into() }
}
...
/// Converts a smoothed signal score into a person count for 3-node ESP32-S3 mesh.
/// Supports 0–3 persons. Uses hysteresis (prev_count) to prevent flickering.
/// `camera_hint`: if Some(n), strongly bias toward that count when confidence is high.
pub fn score_to_person_count(smoothed_score: f64, prev_count: usize) -> usize {
    // With 3 nodes we get triangulated signal: score distribution is more reliable.
    // Allow returning 0 when signal is genuinely absent.
    if smoothed_score < 0.02 {
        return 0;
    }
    match prev_count {
        0 => {
            // Require stronger evidence to start detecting (avoid cold-start false positives)
            if smoothed_score > 0.75 { 2 }
            else if smoothed_score > 0.30 { 1 }
            else { 0 }
        }
        1 => {
            if smoothed_score > 0.80 { 3 }
            else if smoothed_score > 0.65 { 2 }
            else if smoothed_score < 0.02 { 0 }
            else { 1 }
        }
        2 => {
            if smoothed_score > 0.88 { 3 }
            else if smoothed_score < 0.45 { 1 }
            else if smoothed_score < 0.02 { 0 }
            else { 2 }
        }
        _ => {
            // At 3, require strong evidence to drop down
            if smoothed_score < 0.02 { 0 }
            else if smoothed_score < 0.40 { 1 }
...
/// Same as `score_to_person_count` but fused with a camera hint for hybrid mode.
/// `camera_hint`: (count, confidence) from MediaPipe. Confidence 0.0–1.0.
pub fn score_to_person_count_fused(
    smoothed_score: f64, prev_count: usize,
    camera_hint: Option<(usize, f64)>,
) -> usize {
    let wifi_count = score_to_person_count(smoothed_score, prev_count);
    match camera_hint {
        // High camera confidence → trust camera strongly
        Some((cam_count, conf)) if conf > 0.80 => cam_count.min(3),
        // Medium confidence → blend: camera 70% / wifi 30%
        Some((cam_count, conf)) if conf > 0.50 => {
            let blended = cam_count as f64 * 0.70 + wifi_count as f64 * 0.30;
            blended.round() as usize
        }
        // Low/no confidence → rely on WiFi
        _ => wifi_count,
    }
}
