...
pub struct AppStateInner {
    ...
    pub camera_hint: Option<(usize, f64)>,
    pub camera_hint_expiry: Option<std::time::Instant>,
}
...
impl AppStateInner {
    ...
    /// Person count: eigenvalue-based if field model is calibrated, else hybrid
    /// (camera hint + WiFi score fusion).
    pub fn person_count(&self) -> usize {
        use crate::field_bridge;
        use crate::csi::score_to_person_count_fused;

        // Resolve camera hint — expire if TTL has passed.
        let active_hint = match (self.camera_hint, self.camera_hint_expiry) {
            (Some(hint), Some(expiry)) if std::time::Instant::now() < expiry => Some(hint),
            _ => None,
        };
        ...
        match self.field_model.as_ref() {
            Some(fm) => {
                ...
                // Field model gives eigenvalue count; blend with camera hint if present.
                let eigen_count = field_bridge::occupancy_or_fallback(
                ...
                match active_hint {
                    ...
                    Some((cam_count, conf)) if conf > 0.50 => {
                        ((cam_count as f64 * 0.7 + eigen_count as f64 * 0.3).round() as usize).min(3)
                    }
                    _ => eigen_count,
                }
            }
            None => score_to_person_count_fused(
                self.smoothed_person_score, self.prev_person_count, active_hint,
            ),
        }
    }
}
