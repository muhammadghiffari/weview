...
**Date**: 2026-04-25
**Domain**: NV-Diamond Magnetometry × Sensor Simulation × WeView Pipeline Integration
**Status**: Research Survey + Crate Proposal
**Branch**: `research/nv-diamond-sensor-simulator` (no commits, no production code)
**Prior**: `13-nv-diamond-neural-magnetometry.md` framed NV for neural sensing; this doc steps back, surveys what is *actually buildable in 2026*, and asks whether WeView should invest in a Rust simulator crate at all.
...
to WiFi CSI in WeView. That doc projects fT-grade ensemble sensors and helmet-scale
...
given that WeView's primary modality (WiFi-CSI on ESP32-S3) is mature, well-tested, and
shipping.
...
3. Concrete crate proposal *if* WeView decides to build
...
unsuitable for WeView's $200–$500/sensor target.
...
in shielded conditions* but should not be projected onto a $200–$500 deployable WeView
sensor.
...
| Axis | NV-Diamond | OPM | Winner for WeView |
|---|---|---|---|
...
inside `sensor.rs` of a WeView simulator, not be the simulator themselves.
...
**What's missing for WeView**: All of these are *single-sensor, single-defect* tools.
...
This is the layer that would matter most for WeView but is the least developed:
...
- **MEGSIM** / **NeuroFEM** / **MNE-Python forward modelling** — MEG forward models for
  ...
  conductor head models. Overkill for room-scale WeView sensing.
...
This gap is the strongest argument *for* WeView building one.

---

## 4. WeView NV-Diamond Sensor Simulator — Proposal
...
| Use case | Realistic with COTS NV in 2026? | Better answered by | WeView fit |
|---|---|---|---|
...
**The honest reframing**: NV-diamond's WeView niche is **passive magnetic anomaly
...
Biomagnetic neural sensing is a research aspiration, not a 2026 WeView build target.
...
- Single-NV imaging (nm-scale microscopy). Not WeView's geometry.
- NV-NV entanglement protocols. Not WeView's hardware budget.
...
2. Magsim slots cleanly into WeView's existing patterns (proof bundle, frame layout,
...
1. **OPM wins on sensitivity at similar cost** for any biomagnetic use case. If the
   eventual goal is biomag, WeView should simulate OPM, not NV.
...
4. **The hardware decision precedes the simulator**. If WeView is not committing to
...
WeView most needs from a magnetic sensor — for biomag, OPM is better; for ferrous-
...
exotic answer to a question WeView has not yet asked.
...
### 6.1 Is COTS NV noise floor competitive with OPM at WeView's sensor budget?
...
Both are 20–60× over WeView's per-sensor budget, and OPM is ~10⁴× more sensitive
in the biomagnetic band.

**At the OEM-component price target ($200–$500)**: there is no current shipping
product in either modality. No primary source found. Conjecture: WeView would have
...
2026 at WeView's cost target. No primary source for $500-budget NV cardiac sensing
...
found at WeView-relevant frequencies.
...
[Ulrich et al., NDT&E Int. 35, 137 (2002), for civil-engineering NDT — not WeView-
specific]. **No primary source found** for residential-construction magnetic
penetration in the WeView geometry; this is a real research gap.

The dipole 1/r³ attenuation dominates more than wall absorption for WeView room
...
- Systematic measurement of residential-wall magnetic-field penetration loss at
  Hz–kHz frequencies in the WeView geometry (1–10 m sensor-to-source).
...
- Ulrich, R. K. *et al.* on rebar magnetic NDT: *NDT&E Int.* **35**, 137 (2002) —
  cited as proxy for non-WeView-geometry rebar penetration; not directly applicable.
...
simulator crate within the WeView workspace. The build/skip recommendation
