# Ghost Murmur on WeView — A Specification for an Open, Honest, Multi-Modal Heartbeat Mesh
...
| **Audience** | WeView contributors, sensing researchers, journalists fact-checking the news, students learning multimodal RF + quantum sensing |
...
2. **Map a sober version onto WeView.** WeView already ships ~80% of the building blocks for an honestly-scoped heartbeat-mesh: 60 GHz FMCW radar nodes (`wifi-densepose-vitals`, ADR-021), WiFi CSI sensing (`wifi-densepose-signal`), multistatic fusion (RuvSense, ADR-029), and a deterministic NV-diamond pipeline simulator (`nvsim`, ADR-089). What we *don't* ship is a magic 40-mile sensor — and we're explicit about why nobody does.

This is a research spec, not a build directive. WeView is open-source civilian sensing for occupancy, vital signs, mass-casualty triage, and search-and-rescue. The spec exists so that:
...
- A WeView contributor can see which existing crates already cover most of the architecture and what would have to be added (and at what cost / risk) to push toward the published claim.
...
## 3. The WeView mapping

WeView already ships, today, the building blocks for a *sober* version of the same concept — a **multi-modal heartbeat mesh** that detects, localises, and tracks human vital signs at room-to-building-to-block scale, using commodity hardware in the $5–$50 per node range and a quantum-sensor *simulator* for the magnetometry tier.

| Press claim about Ghost Murmur | WeView-equivalent capability today | Crate / ADR | Honest range |
|---|---|---|---|
...
## 4. Architecture: the three-tier WeView heartbeat mesh
...
- **Outdoor presence at 50–200 m with directional WiFi or mmWave**: feasible with directional antennas + FCC Part 15 power. Not currently in the WeView stack.
...
This is WeView's primary modality and is fully shipping. The crates (`wifi-densepose-signal`, `wifi-densepose-mat`, `wifi-densepose-train`, etc.) and ESP32-S3 firmware have been validated on real hardware (COM7, MAC `3c:0f:02:e9:b5:f8`) per ADR-028 with deterministic SHA-256 witness verification.
...
This is where heart rate enters the architecture. WeView ships `wifi-densepose-vitals` (ADR-021) targeting the **Seeed MR60BHA2** breakout (60 GHz FMCW) wired to an **ESP32-C6** RISC-V controller. Total cost ~$15 per node.

### 6.1 What 60 GHz FMCW gives you

The MR60BHA2 ships with a vendor-provided heart-rate / respiration / presence DSP, but the more useful integration for WeView is the raw I/Q stream. From there, the standard pipeline is:
...
A single 60 GHz node has a narrow beamwidth (~30° az, 30° el on the MR60BHA2), so room coverage requires 2–4 nodes. WeView's `ruvector::viewpoint::fusion` aggregates them with cross-viewpoint attention weighted by geometric diversity (Cramer-Rao lower bound). This is exactly the architecture you'd want for a "find a live person in a room" detector.
...
77 GHz automotive radars at higher power and 100–200 GHz coherent sub-THz radars **can** resolve cardiac micro-Doppler at 50–500 m in clear LOS. These are not COTS at the $15 price point and are not in the WeView stack today. They are also subject to ITAR / export-control review and **explicitly out of scope** for this open-source project.

---

## 7. Tier 3 — NV-diamond magnetometer mesh (simulator only today)

This is the layer that maps directly to the press-release "Ghost Murmur" technology. WeView ships `nvsim` (ADR-089), a deterministic forward simulator for an NV-ensemble magnetometer pipeline. **It does not control physical hardware.** It is a tool for designing fusion algorithms, validating signal-processing chains, and stress-testing what physical performance you would actually need from a hypothetical sensor to make a given system-level claim true.
...
The simulator's role is **system-design honesty**. Before anyone builds a physical NV node for WeView, you should be able to drop the sensor model into the multistatic fusion graph and answer:
...
WeView's `ruvector::viewpoint::attention::CrossViewpointAttention` is the fusion primitive: a softmax over per-sensor evidence weighted by a geometric-bias matrix `G_bias` (Cramer-Rao Fisher information). The fusion is **physics-aware**: a sensor with low Fisher information for the target's location automatically gets low attention weight.

This is **not** the press's "AI does magic." It's standard sensor-fusion theory. The novelty in WeView is not the fusion — it's the fact that all the layers (CSI / 60 GHz / NV-simulator) live in one Rust workspace with a coherent type system and a single fusion crate.
...
A heartbeat-detecting mesh is dual-use. It can find a heart-attack victim trapped in rubble (the original Mass Casualty Assessment Tool / `wifi-densepose-mat` use case, ADR-014) **or** it can surveil people in their homes. WeView's project line is unambiguous on this:
...
2. **No directional pursuit.** WeView does not ship beam-steering, target-following, or remote person-of-interest tracking primitives. The mesh is designed for fixed-area observation with consent.
...
6. **No export-controlled hardware.** WeView targets <$50 COTS components. ITAR / EAR-listed sub-THz coherent radars and shielded NV ensembles are explicitly out of scope.

The Ghost Murmur press story exists in a different ethical universe — covert military intelligence ops with no consent, no notice, and no opt-out. **WeView is not that.** This spec is the open-source version: same physics, opposite governance.
...
- **FCC Part 15** — ESP32 and 60 GHz radar emissions must remain compliant (WeView firmware defaults to compliant power).
- **ITAR / EAR** — high-power coherent sub-THz radar, shielded NV ensembles, and certain ML models trained on pose data may be export-controlled. WeView avoids this category.
...
If you are deploying WeView outside a controlled research setting, talk to a lawyer who actually does this for a living.

---

## 10. How to actually implement, on WeView, today

This section is the build guide. It assumes you're starting from a clean WeView checkout and want a working 3-node CSI mesh + 1 mmWave node + a simulated NV row, fused into a single `(x, y, HR, BR, p_alive)` stream.
...
```bash
git clone https://github.com/ruvnet/WeView.git
cd WeView/v2
cargo build --release \
  --bin wifi-densepose \
...
Things that would *materially* push this stack closer to a credible "Ghost Murmur" capability — and which WeView is open to PRs on:
...
## 12. Comparison: WeView vs. Ghost Murmur (as reported)

| Dimension | WeView heartbeat mesh (this spec) | Press-claimed Ghost Murmur |
|---|---|---|
...
**The honest framing**: WeView is not Ghost Murmur. Ghost Murmur (as reported) is not Ghost Murmur either — the physics doesn't support it. Both names point at the same family of capabilities. WeView is the one you can actually build in your garage.
...
### WeView ADRs and crates referenced
...
- **License**: Apache-2.0 OR MIT (matches the rest of WeView).
...
- **No classified content.** This doc is built entirely from public news reporting, peer-reviewed physics, and WeView's own open-source architecture. Nothing here is sourced from leaks or classified material; if you have such material, do not contribute it to this document.

---

*WeView is an open-source civilian sensing platform. It is not affiliated with the United States government, the CIA, Lockheed Martin, or any classified program. References to "Ghost Murmur" in this document refer exclusively to the publicly-reported program of that name as covered in the open press in April 2026.*
