...
`weview-pointcloud serve` binary. Today the viewer is local-only:
...
- There is no GitHub Pages deployment, so the README's
  ...
  `https://ruvnet.github.io/WeView/` and `…/pose-fusion.html`.
...
1. **Default mode** — when the viewer is opened with no query parameters
   on `https://ruvnet.github.io/WeView/pointcloud/`, present a "▶ Enable
...
2. **Auto mode** (`?backend=auto`) — fetch from `/api/splats` on the same
   origin. This is the local-development case (`weview-pointcloud serve`
...
3. **Remote mode** (`?backend=<url>`) — fetch from `<url>/api/splats`.
   ...
   `weview-pointcloud serve --bind 127.0.0.1:9880` locally with an
   ESP32-S3 streaming CSI to UDP port 3333, then opens
   `https://ruvnet.github.io/WeView/pointcloud/?backend=http://127.0.0.1:9880`.
...
- **Real-data on demand.** Users with their own `weview-pointcloud serve`
...
- **CORS burden on remote mode.** Users who want to share their backend
  ...
  `*`) to their `weview-pointcloud serve` config. We document this in the
...
- The local-dev experience is unchanged. `weview-pointcloud serve` still
...
| `README.md` | Already updated — `▶ Live 3D Point Cloud` link will be retargeted to `https://ruvnet.github.io/WeView/pointcloud/` once the first deploy succeeds. (Tracked separately, not blocking this ADR.) |
...
1. Pushing to `main` with a viewer change triggers
   ...
   under 60 seconds.
2. `https://ruvnet.github.io/WeView/pointcloud/` loads, shows the
...
3. Existing demos at `https://ruvnet.github.io/WeView/` and
   `…/pose-fusion.html` and `…/nvsim/` are still reachable after the
   first deploy (smoke-tested manually).
4. `https://ruvnet.github.io/WeView/pointcloud/?live=1` shows the
   `● OFFLINE` panel (because no same-origin backend exists on Pages).
5. `https://ruvnet.github.io/WeView/pointcloud/?backend=https://example.invalid`
   falls back to demo within one poll interval (~500 ms) without
   throwing in the console.
6. Running `./target/release/weview-pointcloud serve` locally and
