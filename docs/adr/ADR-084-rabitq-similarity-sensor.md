...
WeView's signal pipeline already produces several **dense float
...
This ADR formalizes the deployment pattern across the WeView stack and
...
similarity sensor** at four points in the WeView pipeline:
...
- **Sketch type and SIMD primitives**:
  ...
  WeView-flavored API and don't bind directly to the vendor crate.
...
1. **`wifi-densepose-ruvector::sketch` module.** Re-export
   `BinaryQuantized` plus a thin WeView-flavored API
...
- **Does `BinaryQuantized` need a randomized rotation pre-pass for
  WeView's embedding distributions?** Pure sign quantization assumes
