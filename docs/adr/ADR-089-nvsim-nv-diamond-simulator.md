...
landed on a "lean toward skip" verdict for a WeView NV-simulator absent a
...
independent of the rest of the WeView workspace — no internal dependencies
on `wifi-densepose-core`, `wifi-densepose-signal`, or `wifi-densepose-mat`,
because the simulator is generally useful outside WeView's WiFi-CSI
...
- **Standalone leaf**: no internal WeView dependencies, so anyone outside
  WeView can use the crate as-is. WeView integrations land behind opt-in
  feature flags.
...
- **Conjectural propagation defaults**: drywall / brick / dry-concrete
  ...
  residential-wall magnetic-field penetration loss at WeView geometry.
...
- **ADR-066** — Swarm bridge to Cognitum Seed coordinator. If WeView
...
- **Should nvsim be published to crates.io as a standalone crate?** It
  already has no internal WeView deps. The repo's MIT/Apache-2.0
