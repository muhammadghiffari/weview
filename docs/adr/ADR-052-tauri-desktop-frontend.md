# ADR-052: Tauri Desktop Frontend — WeView Hardware Management & Visualization
...
| Issue | [#177](https://github.com/ruvnet/WeView/issues/177) |

## Context

WeView currently requires users to interact with multiple disconnected tools to manage a WiFi DensePose deployment:
...
/// Strategy 1: mDNS — nodes announce _weview._tcp service
...
- **udev rules**: ESP32 serial ports (`/dev/ttyUSB*`, `/dev/ttyACM*`) require udev rules for non-root access. The app bundles a `99-weview-esp32.rules` file and offers to install it:
  ```
...
description = "Tauri desktop frontend for WeView WiFi DensePose"
...
{
  "identifier": "default",
  "description": "WeView default capability set",
  "windows": ["main"],
  "permissions": [
    "core:default",
    "shell:allow-execute",
...
```
~/.weview/nodes.db   (SQLite via rusqlite)
```
...
This desktop tool is quietly becoming the **control plane for WeView**. Once it manages discovery, firmware, OTA, WASM, sensing, and mesh topology, plugin extensibility becomes inevitable:
