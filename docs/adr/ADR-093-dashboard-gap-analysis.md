...
| **Companion** | `assets/NVsim Dashboard.zip` (mockup, ~4200 LOC), live deploy https://ruvnet.github.io/WeView/nvsim/ |
...
| **P0.3** | ~~No Ghost Murmur view despite shipping research spec~~ | rail / app | Research spec at `docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md` had no dashboard surface | ✅ Fixed in `4483a88b2` — new `<nv-ghost-murmur>` component, dedicated rail icon. |
| **P0.4** | Ghost Murmur view is **read-only** | `nv-ghost-murmur.ts` | Currently a static document. The user's directive "fully functional using wasm and weview" requires a live interactive demo. | ⏳ §5 below — interactive distance/moment sliders that actually drive `nvsim::Pipeline` via WASM and report per-tier detectability. |
...
build with `NVSIM_BASE=/WeView/nvsim/` → push to `gh-pages/nvsim/`
...
- `docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md` — Ghost Murmur source material.
- Live deploy — https://ruvnet.github.io/WeView/nvsim/ (verified: rail buttons functional, witness verifies, App Store catalog renders, onboarding tour works).
