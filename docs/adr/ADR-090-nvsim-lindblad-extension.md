...
the **linear regime** — which is the CW-ODMR regime WeView's actual
...
For WeView's CW-ODMR ensemble use case (ferrous-anomaly detection,
...
- **3–7 days of focused work.** The implementation cost is non-trivial:
  ...
  consumer.
- **No current downstream consumer.** WeView's MAT (Mass Casualty
