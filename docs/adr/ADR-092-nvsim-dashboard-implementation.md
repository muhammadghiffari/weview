...
| Field | Value |
|---|---|
| **Status** | **Implemented (2026-04-27)** — live at https://ruvnet.github.io/WeView/nvsim/. PR #436 open against main. 8/12 §11 gates ✅, 4/12 ⚠ (require external infrastructure). |
| **Date** | 2026-04-26 |
| **Authors** | ruv |
| **Refines** | ADR-089 (`nvsim` simulator), ADR-090 (Lindblad extension), ADR-091 (stand-off radar) |
| **Companion** | `assets/NVsim Dashboard.zip` (mockup), `docs/research/quantum-sensing/15-nvsim-implementation-plan.md` (Pass-6 plan), `docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md` (use-case framing) |
...
pattern WeView's WiFi-CSI and 60 GHz vital-signs stacks already follow
...
- **Embeddable**. The dashboard ships as a Vite library build *and* as
  ...
  (e.g. a future WeView fleet console).
...
and integration with the rest of the WeView mesh. The same dashboard,
...
| 11.7 | Offline (PWA) | ✅ | manifest.webmanifest scope `/WeView/nvsim/`, 16 precache entries, workbox autoUpdate SW. |
...
1. **PWA scope on GitHub Pages**: GitHub Pages serves at `/WeView/`
...
WeView already ships 60+ purpose-built edge algorithms. Without an
...
- **`docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md`** — the use-case framing
