...
WeView currently senses the environment using WiFi CSI — a passive technique that analyzes how WiFi signals are disturbed by human presence and movement. While this works through walls and requires no line of sight, CSI-derived vital signs (breathing rate, heart rate) are inherently noisy because they rely on phase extraction from multipath-rich WiFi channels.
...
Add 60 GHz mmWave radar sensor support to the WeView firmware and sensing pipeline with auto-detection and device-specific capabilities.
