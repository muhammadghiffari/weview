# ADR-054: WeView Desktop Full Implementation
...
WeView Desktop v0.3.0 shipped with a complete React/TypeScript frontend but stub-only Rust backend commands. Users report:
...
pub async fn discover_nodes(timeout_ms: Option<u64>) -> Result<Vec<DiscoveredNode>, String> {
    ...
    // 1. mDNS discovery (_weview._tcp.local)
    let mdns = ServiceDaemon::new()?;
    let receiver = mdns.browse("_weview._tcp.local.")?;
