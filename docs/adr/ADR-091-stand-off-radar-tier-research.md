...
| **Companion**  | `docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md` §6.3, ADR-029 (RuvSense multistatic), ADR-089 (nvsim simulator), ADR-090 (Lindblad extension) |
...
ranges in the "tens of miles" against a single human heartbeat. WeView shipped
a research spec (`16-ghost-murmur-weview-spec.md`) which (a) reality-checked the
press claims against published physics, (b) mapped the *honestly-scoped* version
onto the existing WeView three-tier mesh, and (c) explicitly deferred one
...
> are not COTS at the $15 price point and are not in the WeView stack today.
...
WeView's existing modality coverage (per the CLAUDE.md crate table):
...
nodes, the engineering cost is significant — and would require WeView to
...
## 4. Physics ceiling for WeView's heartbeat-mesh use case
...
hyperbole. WeView is not in a position to either confirm or contest the
...
The correct posture for WeView is: **assume the worst case**. If WeView
...
The Ghost Murmur spec (§9) is explicit about WeView's civilian-only ethics
framing:
...
Two specific dual-use risks if WeView research were to ship anything beyond
this ADR:
...
- **Through-wall surveillance**: high-power 77 GHz radar with a wide-band
  ...
  short range. WeView already discloses this capability for the existing
...
- **Target acquisition**: this is the use case WeView explicitly does not
  build for. Period.
...
| 380+ GHz imaging | — | — | **✓ Skip permanently** | Imaging-band, not radar; humidity destroys outdoor link; export-controlled at any meaningful aperture. Not WeView's modality at any plausible build. |
...
exist in WeView today.

## 7. If we research: what does WeView ship?

### 7.1 Mirror the `nvsim` pattern

ADR-089 / 090 established the precedent: when a sensing modality is
*physically interesting but not buildable today*, WeView ships a deterministic
...
2. **A clear non-export-controlled application emerges** — most plausibly
   *medical*: contactless vital-sign monitoring at clinical bedside or
   ...
   the commercial / regulatory path paved by another vendor. WeView would
   then be one of many open-source contributors to a medical sensing modality
   already cleared for civilian use.
3. **WeView core team agrees by RFC**, with explicit sign-off on the dual-use
...
RFC sign-off — WeView still does not ship. The simulator might be expanded;
no firmware ships.
...
- `docs/research/quantum-sensing/16-ghost-murmur-weview-spec.md` — the
...
3. **Is there a clear medical FDA-cleared application for sub-THz cardiac
   ...
   clearance, WeView remains in the simulator-only tier.
4. **Are there current ECCN 6A008 thresholds we should be more concerned
   ...
   confirm.
5. **Should WeView contribute the sub-THz simulator to a neutral upstream**
   (e.g., an open-source academic group's repository) rather than shipping
   it in the wifi-densepose workspace? Decoupling the simulator from WeView
   reduces the risk that future WeView capability work is interpreted as
...
If WeView builds anything in this space, it builds a sub-THz forward
